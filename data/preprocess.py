...
def remove_r1_and_readme():
    origin_path = './mirflickr25k_annotations_v080'
    processed_path = 'mirflickr25k_annotations_v080/remove_r1'
...
def get_tag_map():
    ...
    with open('mirflickr25k/doc/common_tags.txt') as f:
...
def get_label_map():
    ...
    for root, _, filenames in os.walk('./mirflickr25k_annotations_v080/remove_r1'):
...
def process_tags():
    tag2id, id2tag = get_tag_map()
    text_dim = len(tag2id)

    for root, _, filenames in os.walk('mirflickr25k/meta/tags'):
        texts = np.zeros((len(filenames), text_dim), dtype=float)  # BOW
...
def process_labels():
    label2id, id2label = get_label_map()
    label_dim = len(label2id)

    label_map = dict()
    for root, _, files in os.walk('mirflickr25k_annotations_v080/remove_r1'):
        for file in files:
    ...
    total_num = 25000
    labels = np.zeros((total_num, label_dim), dtype=float)  # one-hot labels
...
def process_images():
    ...
    print(f'Saving labels with shape: {images.shape}')
...
def clean():
    ...
    texts = np.load(f'{data_root}/texts.npy')
    labels = np.load(f'{data_root}/labels.npy')
    images = np.load(f'{data_root}/images.npy', allow_pickle=True)
